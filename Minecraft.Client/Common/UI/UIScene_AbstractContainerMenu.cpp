...
UIScene_AbstractContainerMenu::UIScene_AbstractContainerMenu(int iPad, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	m_bMouseLeftDragActive=false;
	m_eMouseDragLastSection=eSectionNone;
	m_iMouseDragLastSlotX=-1;
	m_iMouseDragLastSlotY=-1;
}
	...
	if (getPad() == 0 && m_bMousePointerControl)
	{
		if (KMInput.ConsumeMousePress(0))
		{
			...
			else
			{
				...
				m_bMouseLeftDragActive = true;
				m_eMouseDragLastSection = m_eCurrSection;
				m_iMouseDragLastSlotX = m_iCurrSlotX;
				m_iMouseDragLastSlotY = m_iCurrSlotY;
			}
		}
		else if (m_bMouseLeftDragActive && KMInput.IsMouseDown(0))
		{
			// Fire a click on each new slot the pointer enters while holding left button.
			if (m_eCurrSection != eSectionNone &&
				(m_eCurrSection != m_eMouseDragLastSection || m_iCurrSlotX != m_iMouseDragLastSlotX || m_iCurrSlotY != m_iMouseDragLastSlotY))
			{
				handleKeyDown(m_iPad, ACTION_MENU_A, false);
				m_eMouseDragLastSection = m_eCurrSection;
				m_iMouseDragLastSlotX = m_iCurrSlotX;
